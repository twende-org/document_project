...
import { FaArrowLeft, FaFileInvoice, FaSave, FaPlus, FaTrash, FaDownload, FaEye } from 'react-icons/fa';
import { PDFDownloadLink } from "@react-pdf/renderer";
import type { RootState, AppDispatch } from '../store/store';
import { saveDocument, aiPolishDocument } from '../features/documents/documentsSlice';
import ProformaPDF from "../components/templates/document-templates/ProformaPDF";
...
const ProformaInvoice = () => {
  ...
  React.useEffect(() => {
    if (current && current.doc_type === 'PROFORMA') {
  ...
  const handleSave = async () => {
    dispatch(saveDocument({
      id: current?.id,
      doc_type: 'PROFORMA',
      title: `Proforma Invoice for ${formData.clientName || 'Client'}`,
      content: formData
    })).unwrap()
    .then(() => alert("Proforma Invoice saved successfully!"))
    .catch((err) => alert(err || "Failed to save proforma invoice."));
  };
  ...
  const handlePolish = async () => {
    ...
    .then(() => alert("AI has polished your proforma invoice!"))
        ...
        <header className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="flex flex-col gap-4">
             ...
             <h1 className="text-4xl md:text-6xl font-black text-charcoal dark:text-white uppercase tracking-tighter leading-none">
               Invoice <span className="text-redMain">Architect</span>
             </h1>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
               ...
               <div className="space-y-8">
                 <div>
                   ...
                   <div className="grid grid-cols-1 gap-6">
                     ...
                     <div>
                       <label className="block text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2">Billing Address</label>
                       <input 
                         type="text"
                     ...
                     <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="block text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2">Invoice Date</label>
                          <input 
                            type="date"
                        ...
                        <div>
                          <label className="block text-[10px] font-black uppercase tracking-widest text-gray-400 mb-2">Due Date</label>
                          <input 
                            type="date"
                 ...
                 <div>
                   <div className="flex justify-between items-center mb-8 border-b pb-4">
                     <h3 className="text-sm font-black text-charcoal uppercase tracking-[0.2em]">Services & Items</h3>
                   ...
                   <div className="space-y-4">
                     {formData.items.map((item, idx) => (
                       <div key={idx} className="grid grid-cols-12 gap-3 group animate-fade-in">
          ...
          <aside className={`lg:col-span-7 sticky top-32 transition-all duration-500 ${showPreview ? "block" : "hidden lg:block"}`}>
            <div className="flex items-center justify-between mb-6 px-4">
               ...
               <div className="flex gap-4">
                  <PDFDownloadLink
                    document={<ProformaPDF data={formData} />}
                    fileName={`Proforma_Invoice_${formData.clientName || 'Draft'}.pdf`}
                  >
                    {({ loading: pdfLoading }) => (
                      <Button 
            ...
            <div className="rounded-[2.5rem] bg-slate-200 p-8 shadow-2xl border-4 border-white overflow-hidden min-h-[800px]">
               <div className="bg-white p-12 shadow-inner min-h-[700px] flex flex-col font-sans">
                  <div className="flex justify-between items-start mb-16 border-b-8 border-charcoal pb-12">
                    <div>
                      <h2 className="text-6xl font-black text-charcoal leading-none">PROFORMA</h2>
                      <p className="text-redMain font-black tracking-[0.3em] uppercase text-xs mt-2">Professional Invoice</p>
                    </div>
                    <div className="text-right">
                      <p className="font-black text-charcoal text-xl tracking-tighter uppercase">Twende Documents</p>
                      <p className="text-xs text-gray-400 font-bold uppercase tracking-widest mt-1">Official Document Solutions</p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-16 mb-16">
                    ...
                    <div className="space-y-4 text-right">
                      <div>
                        ...
                        <p className="font-black text-charcoal">{formData.invoiceDate}</p>
                      </div>
                      <div>
                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Terms</p>
                        <p className="font-black text-redMain uppercase tracking-widest">{formData.dueDate}</p>
                      </div>
                    </div>
                  </div>
                    ...
                    <table className="w-full">
                      ...
                      <tbody className="divide-y-2 divide-slate-50">
                        {formData.items.map((item, idx) => (
                          <tr key={idx}>
                            <td className="py-6">
                              <p className="font-black text-charcoal uppercase tracking-tighter text-lg">{item.description || 'Service Description'}</p>
                            </td>
                  ...
                  <div className="mt-16 pt-12 border-t-8 border-slate-50">
                    <div className="flex justify-between items-center bg-charcoal p-10 rounded-[2rem] text-white shadow-2xl">
                      <div>
                        <p className="text-xs font-black uppercase tracking-[0.3em] text-redMain mb-1">Grand Total</p>
                    ...
                    <div className="mt-12 text-center">
                       <p className="text-[10px] font-black text-gray-300 uppercase tracking-[0.4em]">Generated via Twende Documents Precision Engine</p>
                    </div>
                  </div>
               </div>
            </div>
