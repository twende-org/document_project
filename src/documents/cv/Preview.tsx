  ...
  if (layout === 'modern') {
    return (
      <div className="bg-slate-200 p-4 md:p-8 rounded-[2.5rem] shadow-2xl border-4 border-white overflow-hidden min-h-[800px] sticky top-32 text-left">
        <div className="bg-white shadow-inner min-h-[700px] flex flex-col font-sans text-charcoal p-12">
           <header className="mb-12 flex justify-between items-end border-b-4 pb-8" style={{ borderBottomColor: primaryColor }}>
              <div>
                <h2 className="text-5xl font-black text-charcoal uppercase tracking-tighter leading-none mb-2">{personalInfo.fullName}</h2>
                <p className="text-xl font-bold text-gray-400 uppercase tracking-tight">{personalInfo.jobTitle}</p>
              </div>
              <div className="text-right text-[10px] font-black uppercase tracking-widest text-gray-400 space-y-1 max-w-[200px]">
                 <p className="truncate">{personalInfo.phone}</p>
                 <p className="truncate">{personalInfo.email}</p>
                 <p className="truncate">{personalInfo.address}</p>
              </div>
           </header>
           ...
           <div className="grid grid-cols-12 gap-12">
              <div className="col-span-8 space-y-12">
                 <section>
                    <p className="text-sm text-gray-500 font-medium leading-relaxed border-l-4 pl-6" style={{ borderLeftColor: primaryColor }}>{summary}</p>
                 </section>
                 <section>
                    <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-300 mb-6">Professional History</h4>
                    <div className="space-y-8">
                       {experience.map((exp: any, i: number) => (
                         <div key={i}>
                            <div className="flex justify-between items-baseline mb-2">
                               <h5 className="font-black text-charcoal uppercase text-lg">{exp.title}</h5>
                               <span className="text-[10px] font-black text-gray-400">{exp.duration}</span>
                            </div>
                            <p className="text-xs font-black uppercase tracking-widest mb-3" style={{ color: primaryColor }}>{exp.company}</p>
              ...
              <div className="col-span-4 space-y-12">
                 <section>
                    <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-300 mb-6">Expertise</h4>
                 ...
                 <section>
                    <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-300 mb-6">Academic Background</h4>
                    <div className="space-y-4">
                       {education.map((edu: any, i: number) => (
                         <div key={i}>
                            <p className="text-xs font-black text-charcoal uppercase">{edu.degree}</p>
                            <p className="text-[10px] font-bold text-gray-400">{edu.school}</p>
                         </div>
                       ))}
                    </div>
                 </section>
